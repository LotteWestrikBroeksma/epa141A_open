...
def new_get_model_for_problem_formulation(problem_formulation_id):
    ...
    problem_formulation_id : int {0, ..., 5}
                             ...
                             6: total cost & causalities, expected damages & causalities
    ...
    # Fully disaggregated:
        ...
        total_cost_variables = []
        total_casualty_variables = []
        damage_variables = []
        casuality_variables = []

        total_cost_variables.extend(
            [
                f"{dike}_{e}"
                for e in ["Expected Annual Damage", "Dike Investment Costs"]
                for dike in function.dikelist
        ...
        total_cost_variables.extend([f"RfR Total Costs"])
        total_cost_variables.extend([f"Expected Evacuation Costs"])

        dike_model.outcomes = [
            ScalarOutcome(
                "All Costs",
                variable_name=[var for var in total_cost_variables],
                function=sum_over,
                kind=direction,
            ),
            ScalarOutcome(
...
if __name__ == "__main__":
    new_get_model_for_problem_formulation(3)